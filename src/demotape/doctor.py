"""Environment diagnostic — run this first when a render fails.
    ...
    python -m demotape.doctor            # check the playwright backend
    python -m demotape.doctor --xvfb     # also check the xvfb stack
    ...
    if which("ffmpeg"):
        ...
        if bundled:
            ...
            r.add(
                ...
                'pip install "demotape[bundled-ffmpeg]"',
            )
...
def check_capture(r: Report) -> None:
    ...
    if not (which("Xvfb") and which("ffmpeg")):
        return
    display = ":98"
    out = Path("/tmp/demotape_doctor.mp4")
